    ...
    <div className='h-screen flex items-center justify-center w-full bg-purple-800'>
        {!steps?<div className='h-52 w-52 flex flex-col gap-2 justify-center items-center p-5 rounded-md bg-white'>
            ...
            <div className='flex flex-col gap-2 justify-center items-center'>
                ...
                {steps?.map((step)=>(<div className='flex gap-5 justify-between font-bold' key={step._id}><p className='bg-gray-400 flex items-center rounded-md px-2 py-1'>{step.goal}</p><div className='flex gap-1 items-center justify-center bg-[#0f5a2d62] px-2 py-1 rounded-md'><img src="/assets/check.svg" alt="" /><p>Tracked</p></div></div>))}
            </div>
        </div>}
