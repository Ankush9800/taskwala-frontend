...
function Submission() {
  ...
  const filteredSubmissions = submissions?.filter(submission => {
    const matchesSearch = submission.phone?.includes(searchTerm) ||
                         submission.upi?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         submission.redirectUrl?.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const stats = {
    ...
    pending: submissions?.filter(s => s.status?.toLowerCase() === 'pending').length || 0,
    approved: submissions?.filter(s => s.status?.toLowerCase() === 'approved').length || 0,
    totalPayout: submissions?.reduce((sum, s) => sum + (parseFloat(s.payoutRate) || 0), 0) || 0
  };

  const getStatusBadge = (status) => {
    switch (status?.toLowerCase()) {
      case 'approved':
        return <Badge className="bg-[#10B981] text-white"><CheckCircle className="w-3 h-3 mr-1" />Approved</Badge>;
      case 'pending':
